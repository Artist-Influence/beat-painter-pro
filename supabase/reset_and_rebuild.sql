-- =============================================================================
-- Beat Painter Pro — RESET v1 → clean v2 schema.
-- DESTRUCTIVE: drops all v1 app tables + their data, then rebuilds clean.
-- Run once in your project's SQL Editor. (Auth users are NOT touched here — if you
-- also want a clean login slate, delete them in Authentication → Users.)
-- =============================================================================

-- ---- Drop v1 app objects ----------------------------------------------------
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

DROP TABLE IF EXISTS public.custom_visualizers   CASCADE;
DROP TABLE IF EXISTS public.uploaded_visualizers CASCADE;
DROP TABLE IF EXISTS public.user_sessions        CASCADE;
DROP TABLE IF EXISTS public.visualizer_events    CASCADE;
DROP TABLE IF EXISTS public.visualizer_leads     CASCADE;
DROP TABLE IF EXISTS public.visualizer_templates CASCADE;
DROP TABLE IF EXISTS public.user_roles           CASCADE;
DROP TABLE IF EXISTS public.profiles             CASCADE;

DROP FUNCTION IF EXISTS public.check_visualizer_quota(uuid);
DROP FUNCTION IF EXISTS public.get_visualizer_count(uuid);
DROP FUNCTION IF EXISTS public.get_daily_visualizer_count(uuid);
DROP FUNCTION IF EXISTS public.enforce_custom_visualizer_update();
DROP FUNCTION IF EXISTS public.handle_new_user();
DROP FUNCTION IF EXISTS public.has_role(uuid, public.app_role);
DROP FUNCTION IF EXISTS public.update_updated_at_column();
DROP TYPE IF EXISTS public.app_role;

-- ---- Rebuild clean (full schema below) --------------------------------------
-- =============================================================================
...
-- Let functions reference tables that are created later in this script.
...
-- ---- Helper function (table-independent) ------------------------------------
...
-- ---- Functions that reference tables (now that the tables exist) ------------
